...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter((e: (Block, List[Move])) => !explored.contains(e._1))
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.Empty
      ...
      val nno = newNeighborsOnly(neighborsWithHistory(block, history), explored)
      val exploredUpdated = explored.union(nno map (_._1) toSet)
      (block, history) #:: from(initial.tail #::: nno, exploredUpdated)
    }
